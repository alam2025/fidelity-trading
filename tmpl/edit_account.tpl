...
<tr>
 <td>Account Name:</td>
 <td><input type=text  value='{$userinfo.username}' disabled class=inpts size=30></td>
</tr><tr>
 <td>Registration date:</td>
 <td><input type=text  value='{$userinfo.date_register}' disabled class=inpts size=30></td>
</tr><tr>
...
<tr>
 ...
 <td>{if $settings.usercanchangeegoldacc == 0 && $ps.account != ''}{$ps.account|escape:html}{else}<input type=text disabled class=inpts size=30 name="pay_account[{$ps.id}]" value="{$frm.pay_account[$ps.id]|default:$ps.account|escape:html}" data-validate="{$ps.validate.func}" data-validate-{$ps.validate.func}="{$ps.validate[$ps.validate.func]}" data-validate-notice="{$ps.validate.notification|escape:html}">{/if}</td>
</tr>
{/foreach}
...
<tr>
 ...
 <td>{if $settings.usercanchangeegoldacc == 0 && $ps.value != ''}{$ps.value|escape:html}{else}<input type=text disabled class=inpts size=30 name="pay_account[{$p.id}][{$ps.name|escape:html}]" value="{$frm.pay_account[$p.id][$ps.name]|default:$ps.value|escape:html}">{/if}</td>
</tr>
{/foreach}
{/foreach}
<tr>
